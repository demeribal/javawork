    ...
    products.forEach(product => {
        product.addEventListener('click', function() {
            ...
            const productPrice = parseFloat(
                this.querySelector('.product-price').textContent.replace(/₩/g, 
                    '').replace(/,/g, '').trim());
            ...
            // sessionStorage에 저장 (키: productData)
            const existingData = JSON.parse(sessionStorage.getItem("productData")) || [];
            existingData.push({ name: productName, price: productPrice });
            sessionStorage.setItem("productData", JSON.stringify(existingData));
