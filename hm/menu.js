...
function initMenuPage() {
    ...
    function fetchMenuData() {
        fetch("http://localhost:8080/api/menus")
            ...
            .then(data => {
                ...
                // 중요: 'use' 필드를 'isUse'로 변환
                const mappedData = data.map(menu => {
                    menu.isUse = menu.use; // 필드명 매핑
                    return menu;
                });
                
                displayMenus(mappedData);
            })
            .catch(err => {
    ...
    function updateMenuStatus(id, isActive) {
        ...
        fetch(`http://localhost:8080/api/menus/${id}`)
            ...
            .then(menu => {
                // 중요: 백엔드의 'use' 필드 사용
                menu.use = isActive; // 필드명 매핑
                
                return fetch(`http://localhost:8080/api/menus/${id}`, {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json"
                    },
    ...
    function displayMenus(menus) {
        ...
        if (menus.length === 0) {
            ...
            emptyRow.innerHTML = '<td colspan="6" class="text-center">데이터가 없습니다.</td>';
            ...
            row.innerHTML = `
                ...
                <td>
                    <input type="checkbox" class="checkbox" 
                        data-id="${menu.id}" ${menu.isUse ? 'checked' : ''}>
                </td>
                <td>
