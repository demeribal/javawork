...
function updateSelectionUI() {
  ...
  const slotBoxes = document.querySelectorAll('.slot-box');
  const currentBox = slotBoxes[currentSlotIndex];
  if (currentFlavors.length < currentProduct.flavorsRequired && currentBox.hasChildNodes) {
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll('.flavor-item').forEach(flavor => {
  ...
  // API 주소 확인
    ...
    .then(data => {
      ...
      `;      
        ...
        item.addEventListener("click", e => onFlavorItemClick(e));
